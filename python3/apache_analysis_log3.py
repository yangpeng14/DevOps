...
# Employ:       python3 apache_analysis_log3.py ApacheLogFilePath or python3 apache_analysis_log3.py ApacheLogFilePath number
# ------------------------------------------------------
...
class analysis_log():
    ...
    def split_eachline_todict(self, line):
        ...
        judgement_ip = re.compile(r'(?<![\.\d])(?:\d{1,3}\.){3}\d{1,3}(?![\.\d])')
        ip_list = re.findall(judgement_ip, str(split_line))
        #判断ip_list原因：因为有时日志中没有IP地址，比如有时post日志中就没有
        if ip_list:
            if len(ip_list) == 2:
                split_dict = {'remote_host': ip_list[0], 'status': split_line[9], 'bytes_sent': split_line[10]}
            elif len(ip_list) == 3:
                split_dict = {'remote_host': ip_list[0], 'status': split_line[10], 'bytes_sent': split_line[11]}
            elif len(ip_list) == 4:
                split_dict = {'remote_host': ip_list[0], 'status': split_line[11], 'bytes_sent': split_line[12]}
            elif len(ip_list) == 5:
                split_dict = {'remote_host': ip_list[0], 'status': split_line[12], 'bytes_sent': split_line[13]}
            else:
                split_dict = {'remote_host': ip_list[0], 'status': split_line[8], 'bytes_sent': split_line[9]}
            return split_dict
